...
adjust_settings() {
    ...
    if [[ -s ".config" ]]; then
        ...
        sed -i "s/CONFIG_TARGET_ROOTFS_PARTSIZE=.*/CONFIG_TARGET_ROOTFS_PARTSIZE=768/" .config
...
custom_packages() {
    ...
    other_packages=(    
        ...
        #"luci-app-cpufreq|https://dl.openwrt.ai/packages-24.10/$ARCH_3/kiddin9"
...
rebuild_firmware() {
    ...
    misc+=" luci-app-temp-status luci-theme-alpha luci-app-droidnet luci-app-rakitanmanager"
    ...
    PACKAGES+=" $misc zram-swap adb parted losetup luci luci-ssl block-mount luci-app-poweroff luci-app-ramfree htop bash curl wget-ssl unzip jq luci-app-ttyd nano luci-app-tinyfm luci-app-eqosplus luci-app-modeminfo"
